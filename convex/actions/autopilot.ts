...
export const processBatch = internalAction({
    ...
    handler: async (ctx) => {
        ...
        const activeCampaigns = await ctx.runQuery(internal.campaigns.getActiveAutopilotCampaigns);
        ...
        for (const campaign of activeCampaigns) {
            ...
            const leads = await ctx.runQuery(internal.leads.getPendingLeadsForCampaign, {
            ...
            for (const lead of leads) {
                // 3. Generate message
                const strategy = await ctx.runQuery(internal.strategies.getStrategy, { strategyId: campaign.strategyId });

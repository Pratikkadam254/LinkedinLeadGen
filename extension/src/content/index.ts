...
import { injectOverlayUI, updateOverlayProgress, showOverlaySummary, removeOverlay } from './injector';
...
let voyagerLeadCache: Map<string, Partial<ExtractedLead>> = new Map();
  ...
  const callbacks: ExtractionCallbacks = {
    ...
    onComplete: async (allLeads) => {
      ...
      showOverlaySummary({
        ...
        duration: Date.now() - (extractionConfig as any).startTime || 0,
